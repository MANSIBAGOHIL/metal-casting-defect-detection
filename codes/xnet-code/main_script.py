...
# Modified evaluate_model function to handle memory better
def evaluate_model(model, test_loader, device='cuda', batch_size=32):
    model.eval()
    all_preds = []
    all_labels = []
    
    # Create class name mapping for visualization
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(test_loader, desc="Evaluating"):
            ...
            # Convert to python lists immediately to free up GPU memory
    ...
    np.save('prediction_results.npy', prediction_data)
    
    # Calculate per-class accuracy
    plt.figure(figsize=(10, 6))
    class_accuracy = cm.diagonal() / cm.sum(axis=1)
    
    # Create bar chart of per-class accuracy
...
def visualize_predictions(model, test_loader, device='cuda', num_images=5):
    model.eval()
    ...
    # Create denormalization function (reverse of normalization in transforms)
    ...
    for idx in range(num_images):
        ...
        img = denormalize(images[idx]).cpu().numpy().transpose((1, 2, 0))
...
def main():
    ...
    transform = transforms.Compose([
        ...
        transforms.Resize((224, 224)),  # Reduced size for efficiency
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])
    
    # Create dataset - update the path to your dataset
    root_dir = 'D:/Capston Project/Sem_8/GC10-DET/classification'  # Change this to your actual dataset path
    ...
    # Create data loaders with increased batch size for evaluation
    ...
    visualize_predictions(model, test_loader, device=device, num_images=8)
